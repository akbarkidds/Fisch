...
local Highlight =
	loadstring(
		game:HttpGet("https://raw.githubusercontent.com/akbarkidds/Fisch/refs/heads/main/Tools/highlight.lua")
	)()
...
function remoteHandler(hookfunction, methodName, remote, args, funcInfo, calling, returnValue)
    ...
    if validInstance and validClass then
        ...
        if not calling then
            _, calling = pcall(getScriptFromSrc, funcInfo and funcInfo.source)
        end
        coroutine.wrap(function()
        ...
        if func then
            ...
            -- Safely try to get constants if supported
            pcall(function()
            ...
            -- Safely convert function info to string
            pcall(function()
...
function hookRemote(remoteType, remote, ...)
    if typeof(remote) == "Instance" then
        ...
        if validInstance and not (blacklist[remote] or blacklist[remoteName]) then
            ...
            -- Handle RemoteFunction with return values
            if recordReturnValues and remoteType == "RemoteFunction" then
                local thread = coroutine.running()
                if not thread then return end -- Safety check
                
                local success, err = pcall(function()
                    task.spawn(function() -- Using task.spawn instead of task.defer for better compatibility
                        local returnValue
                ...
                -- Handle RemoteEvent or non-return value RemoteFunction
                schedule(
                    remoteHandler,
                    true,
                    remoteType == "RemoteEvent" and "fireserver" or "invokeserver",
    ...
    -- Handle the actual remote calls
    if recordReturnValues and remoteType == "RemoteFunction" then
        return coroutine.yield()
