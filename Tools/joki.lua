...
function WebhookManager(WebhookUrl, Title)
    spawn(function()
        ...
        if Title == nil then Title = "Fisch" end
        local Embed = {
...
function Save(datax)
    if isfile(fullPath) then
        ...
        if tonumber(Money[1]) >= tonumber(coin) then
            delfile(fullPath)
            WebhookManager("https://discordapp.com/api/webhooks/1338037844848939029/84Em4gtCGvPOIMKwbvQKMh1e3kit_SGl81DPkjBPbje2I2DqddurnEUr3PHwAlYSW_Kd", "Joki Fisch")
